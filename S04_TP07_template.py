...
class Grid:
    ...
    def __init__(self, grid_init):
        ...
        self.columns_count = len(grid_init[0])

    def fill_random(self, values):
        """ Rempli la grille de valeurs aléatoires de la liste 'values'"""
        self.grid = [[random.choice(values) for _ in range(
            self.columns_count)] for _ in range(self.lines_count)]
    ...
    def get_anti_diagonal(self):
        ...
        res = []
        line_init = 0
        grid_init = len(self.grid[0]) - 1

        for line in range(len(self.grid)):
            for col in range(len(self.grid[line])):
                if (line_init > line):
                    break
                res.append(self.grid[line_init][grid_init])
                line_init += 1
                grid_init -= 1
    ...
    def get_coordinates_from_cell_number(self, cell_number):
        ...
        for l in range(len(self.grid)):
            col = 0
            for _ in range(len(self.grid[l])):
                if i == cell_number:
                    break
                col += 1
                i += 1
