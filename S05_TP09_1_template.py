...
class Grid:
    ...
    def get_neighbour(self, line_number, column_number, delta, is_tore=True):
        ...
        new_line_number, new_column_number = line_number + \
    ...
    def get_cell_neighborhood_numbers(self, cell_number, deltas, is_tore=True):
        ...
        res = []
        for delta in deltas:
            neighbour = self.get_cell_neighbour_number(
                cell_number, delta, is_tore)
...
if __name__ == '__main__':
    ...
    assert GRID_TEST.get_cell_neighbour_number(
        CELL_NUMBER_TEST, DIRECTION_TEST, IS_TORE_TEST) == 7
    assert not GRID_TEST.get_cell_neighbour_number(
        CELL_NUMBER_TEST, DIRECTION_TEST, not IS_TORE_TEST)
    assert GRID_TEST.get_cell_neighborhood_numbers(
        CELL_NUMBER_TEST, WIND_ROSE, IS_TORE_TEST) == [0, 5, 6, 7, 12, 14, 19, 20]
    assert GRID_TEST.get_cell_neighborhood_numbers(
        CELL_NUMBER_TEST, WIND_ROSE, not IS_TORE_TEST) == [5, 6, 12, 19, 20]
    print("Tests all OK")
